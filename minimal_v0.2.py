...
def create_rectangle_nail_positions(shape, total_nails=200):
    ...
    # Calculate perimeter and step size based on desired total nails
    perimeter = 2 * (height + width - 2)  # -2 to avoid double counting corners
    nail_step = max(1, perimeter // total_nails)
    ...
    # If we have too many nails, trim to exact count
    if len(nails) > total_nails:
        # Distribute evenly by taking every nth nail
        step = len(nails) // total_nails
        nails = nails[::step]
        if len(nails) > total_nails:
            nails = nails[:total_nails]
...
def create_circle_nail_positions(shape, total_nails=200, r1_multip=1, r2_multip=1):
    ...
    radius_y = (min(height, width) // 2 - 1) * r1_multip
    radius_x = (min(height, width) // 2 - 1) * r2_multip
    
    # Create nails at equal angular intervals
    nails = []
    for i in range(total_nails):
        angle = 2 * np.pi * i / total_nails
        y = int(centre[0] + radius_y * np.sin(angle))
        x = int(centre[1] + radius_x * np.cos(angle))
